 ...
 *         @Vich\Uploadable
 */
class UserGravatar extends BaseUser
{
    ...
    public function setEmail($email)
    {
        ...
        if ($email != '' ||($this->getAvatar() == '' ||
                substr($this->getAvatar(), 0, strlen(self::GRAVATAR )) == self::GRAVATAR))
        {
            $this->setAvatar('');
        }
        return $this;
    ...
    {
        if ($this->getEmail() != '' && $avatar == '')
        {
            $avatar = self::GRAVATAR  . md5(strtolower(trim($this->getEmail())));
        }
        $this->avatar = $avatar;
